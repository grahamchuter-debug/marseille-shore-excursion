...
export interface RelatedGuideLike {
  slug: string;
  title: string;
  tagline: string;
  hubPath: string;
...
interface GuideArticleProps {
  page: GuidePage;
  hubLabel: string;
  getRelatedPage: (slug: string) => RelatedGuideLike | undefined;
}

export function GuideArticle({ page, hubLabel, getRelatedPage }: GuideArticleProps) {
  const path = `${page.hubPath}/${page.slug}`;
  const image = subjectImages[page.imageKey] ?? subjectImages.fortifications;
  const related = page.relatedSlugs.map(getRelatedPage).filter(Boolean) as RelatedGuideLike[];

  const breadcrumbs = [
    { name: "Home", path: "/" },
    { name: hubLabel, path: page.hubPath || "/guides" },

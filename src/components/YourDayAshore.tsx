      ...
      <div className="container-wide">
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {dayAshoreItems.map((item) => (
            <article
              key={item.id}
              className="rounded-2xl border border-coastal-100/90 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md sm:p-7"
            >
              <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-coastal-50 text-coastal-700 ring-1 ring-coastal-100">
                <DayAshoreIcon type={item.icon} />
              </span>
              <h3 className="mt-4 font-display text-lg font-semibold text-gray-900">{item.title}</h3>

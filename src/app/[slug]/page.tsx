...
import { highlightPageMetadata, guidePageMetadata, comparisonPageMetadata } from "@/lib/seo";
...
export function generateStaticParams() {
  return getAllFlatPageSlugs().map((slug) => ({ slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (flat.kind === "highlight") {
    return highlightPageMetadata(slug, flat.page.seoTitle, flat.page.metaDescription);
  }
  if (flat.kind === "experience") {
    return guidePageMetadata(flat.page.hubPath, slug, flat.page.seoTitle, flat.page.metaDescription, flat.page.imageKey);
  }
  return comparisonPageMetadata(slug, flat.page.seoTitle, flat.page.metaDescription);
}

function ComparisonPageBody({ slug }: { slug: string }) {
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {isGuide
              ...
              : comp.relatedSlugs.map((s) => (
                  <Link key={s} href={`/${s}`} className="btn-secondary text-sm">
                    Related comparison
                  </Link>
                ))}
          </div>
...
export default async function FlatEditorialPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const flat = getFlatPage(slug);
  if (!flat) notFound();

  if (flat.kind === "highlight") {
    return <HighlightArticle page={flat.page} />;
  }

  if (flat.kind === "experience") {
    return (
      <GuideArticle page={flat.page} hubLabel="Guides" getRelatedPage={getRelatedGuideLike} />
    );
  }

  return <ComparisonPageBody slug={slug} />;
}

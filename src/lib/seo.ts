...
/**
 * `hubPath` is retained for backward compatibility but every current guide's
 * `hubPath` is `""`, so this already resolves to the flat `/${slug}` path.
 */

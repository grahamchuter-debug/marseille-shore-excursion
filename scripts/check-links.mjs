...
// schedules.ts does not exist on this site (no ship-schedules feature) — kept
// harmless since loadSlugsFromTs resolves to [] and the patterns below never match.
...
// Flat editorial pages (highlights, experiences and comparisons) all resolve
// to /{slug} via the catch-all src/app/[slug]/page.tsx route.
for (const slug of excursionSlugs) validPaths.add(`/shore-excursions/${slug}`);
for (const slug of [...highlightSlugs, ...experienceSlugs, ...comparisonSlugs]) validPaths.add(`/${slug}`);
